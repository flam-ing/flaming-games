...
가정용·휴대용 본편/외전/재수록판 18개에서 작품별 40개를 선정했고, Mario Party-e 11개와 Jamboree TV의 추가 20개를 별도 컬렉션에 포함했습니다. **선정한 751개 수록 항목 전부를 플레이할 수 있습니다.** 재수록판에서 같은 게임을 다시 선정한 항목도 포함하는 수입니다. 18개 작품은 각각 40개, Mario Party-e는 11개, Jamboree TV 확장은 20개입니다. 홈페이지는 실제 등록한 구현과 검증한 원작 버전을 연결합니다.

재수록판에는 원작과 다른 타이머, 점수, 판정이 존재합니다. `registry.js`의 `supportedTitles` 허용 목록으로 판별하며, 이름이 같다는 이유만으로 검증하지 않은 버전을 공개하지 않습니다. UI는 수록 항목 수와 중복 제목을 제외한 수를 구분합니다. 같은 제목이어도 규칙이 다른 경우가 있으므로, 중복 제목을 제외한 수가 서로 다른 규칙의 정확한 개수는 아닙니다.
...
2026-09-20 구현 완료 버전에서 전체 751개 수록 항목의 2개 시드 경기 시뮬레이션 1,502회와 실제 Chrome의 751개 시작·입력·3D 렌더 검사를 통과했습니다. 모바일, 솔로 기록 결과, 일시정지·재시작, 대표 포인터·팀 조작은 별도로 확인했습니다.
...
시뮬레이션 검사는 등록된 모든 수록 버전을 두 시드에서 실행해 자연 종료, 결과 인덱스, 좌표·점수·렌더 파라미터의 유한성, 제한시간을 확인합니다. 원작에 고정 시간이 없는 게임은 대체로 무제한으로 진행하며 테스트 실행에 상한을 둡니다. 출처가 숨은 종료시간의 수치를 명시하지 않아 브라우저용 상한을 둔 경우에는 게임별 안내에 그 차이를 표시합니다. 결과는 Git에서 제외한 `scripts/out/simulation.json`에 저장합니다. 원작 규칙 대조 및 실제 브라우저 렌더·입력·모바일 검증은 이 검사와 별도로 수행합니다.
